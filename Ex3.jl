...
function graph_constructor(inst)
    ...
    for i in 1:n_vertices, j in 1:n_vertices 
        if i!= j
            push!(A[i],Edge(i,j,-1,[inst.parts_volume[j], inst.parts_weight[j], inst.distance[i,j]/inst.avg_speed]))
        end
    end
    #Add dummy edge
    for i in 2:n_vertices
        push!(A[i],Edge(i,n_vertices+1,0, [0,0, inst.distance[i,1]/inst.avg_speed] ))
    end
